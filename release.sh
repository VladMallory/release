...
echo -e "${YELLOW}📤 Отправляем изменения в main...${NC}"
...
if [[ $? -ne 0 ]]; then
    ...
    if [[ $force_push == "y" || $force_push == "Y" ]]; then
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${RED}❌ Ошибка при принудительной отправке в main${NC}"
            exit 1
        fi
        echo -e "${GREEN}✅ Изменения принудительно отправлены в main${NC}"
    else
        echo -e "${YELLOW}⚠️  Отправка в main отменена${NC}"
        echo -e "${YELLOW}💡 Рекомендуется выполнить 'git pull origin main' для синхронизации${NC}"
    fi
fi
